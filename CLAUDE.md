...
- **Passo 2 do comparador (19/09/2026, Everton):** o "tipo de negócio" é palpite nosso, não dado. O padrão agora compara **uma forma de pagar de cada vez** (`modo`: `debito`, `credito_avista` — padrão —, `credito_parcelado` sempre em 12x) com 100% do valor; segmento e mix manual só existem em **"Simulação avançada"** (`modo = avancado`). URL: `pg=` (só entra se diferente do padrão); `s`, `mix`, `px`, `vm` só são gravados no modo avançado, e link antigo com eles abre como avançado.
- **Ticket médio saiu da tela.** Ele só servia para contar transações quando a taxa cobra valor fixo por venda, e nenhuma taxa cadastrada tem `valor_fixo` (0 de 890 em produção, 19/09/2026). O motor continua suportando `valor_fixo`; só a pergunta saiu.

- **Selos "melhor para…" (19/09/2026):** `selosDoItem()` em `comparador.js` — "Menor débito", "Menor crédito à vista", "Menor crédito 12x" e "Sem mensalidade". Saem da taxa publicada (grupo padrão, no prazo que o cartão usa), nunca do mix. Só entre planos permanentes do bloco calculado; empate divide o selo; selo que todos ganhariam não aparece; "Sem mensalidade" só aparece se alguma concorrente cobra.
...
### Adesão sempre pelo link, prazo sem "falta dado" falso, Yelly sem código (19/09/2026, Everton)

- **A adesão mostrada é SEMPRE a que o cliente paga entrando pelo link do Everton.** Quando a página do link tem preço próprio (não é "site menos o percentual do cupom"), ele vive em `equipamento_plano.preco_adesao_no_link` (campo "Adesão pelo link de afiliado" no painel, aba Equipamentos do plano). O motor (PHP `orcamentoDoAparelho` e JS `motor.mjs`) usa esse valor como `valor_final` quando há cupom aplicado; `EconomiaDoCupom` também. Nulo = vale a conta de sempre pelo desconto do cupom. **Ao cadastrar uma marca nova, abrir o link de afiliado e conferir se o preço da página bate com "site menos o cupom"; se não bater, preencher o preço no link.** Até hoje só a FacilityPay divergia: Mini R$ 55,50 (confirmado em captura, planos D1PLUS e EXPRESS), Pro R$ 119,90 e Smart R$ 221,90 (da leitura de 16/09, não reconferidos por plano) — nos três planos do catálogo. Antes disso o comparador mostrava R$ 94,41 (Express) / R$ 58,41 (Light), que eram o preço do site menos 10%.
- **Plano que não recebe no prazo escolhido não aparece — nem como "falta dado".** `MotorDeCalculo::planoOfereceOPrazo` (espelho em `motor.mjs`): com prazo pedido e ao menos uma venda de cartão no cenário, o plano sai do resultado se não tem NENHUMA taxa de cartão nesse prazo (o Pix cai sempre na hora e não conta). "Falta dado" fica só para plano que oferece o prazo mas não publicou uma taxa específica. Efeito colateral assumido: a regra 4 ("marca não some") vale quando o prazo é "tanto faz"; com prazo pedido, a marca que não vende nele some — foi o que o Everton pediu. Cenário só de Pix ignora o prazo.
- **Cupom de código genérico (`cupons.codigo_generico`):** o código vale para qualquer afiliado (Yelly: `AFILIADOS10`), então digitá-lo no site oficial não credita a comissão. O portal não exibe nem manda copiar o código (cartão do comparador, `x-bloco-cupom`, páginas do cupom e da marca); o desconto de 10% continua contando, porque o link já o aplica. O código segue no banco e na URL de saída para o rastreio de clique. Marcado no `CuponsAfiliadoSeeder`; em produção via tinker (o reseed não é o caminho — ver a nota sobre `delete()` em seeders).
- **Modal da tabela promocional** não mostra mais "Custo inicial nesta tabela". **Nota do Reclame Aqui** agora fica na mesma linha, em negrito, do "Link de parceiro" (ou da adesão, para marca sem parceria) — componente `x-nota-reclame-aqui`. **Botão "Simulação avançada"** ganhou borda azul-marinho, sombra e ícone de controles para se destacar dos três modos simples.

### Regra de paridade computador/celular e ajustes de 19/09/2026 (noite)

- **REGRA (Everton, 19/09/2026): toda alteração feita para quem usa computador reflete igual no celular.** Texto, link, rótulo, destino e regra de negócio são um só; só o layout pode variar. O único elemento exclusivo do celular é a barra fixa do 1º colocado (`comparador.blade.php`, `sm:hidden`) — e ela agora usa exatamente `textoContratar`/`hrefContratar`/`temCupomParaContratar` do cartão (antes tinha `textoContratarCurto` "Contratar"/"Ir para X", que divergia). Antes de fechar qualquer mudança de tela, procurar `sm:hidden`/`hidden sm:` e conferir. Coberto por `CtaIgualNoCelularTest`.
- **O CTA vai SEMPRE pelo link de afiliado quando a marca tem cupom vigente** (`cupomDaMarca()` em `comparador.js`), mesmo com "Considerar cupons de desconto" desmarcado. Desmarcar só muda a conta da adesão (sem o desconto), nunca o destino do botão. Antes, com o checkbox desmarcado, `temCupomParaContratar` dava falso e o botão virava "Ir para o site da Ton" direto ao site oficial.
- **Marcas sem resultado (`marcasSemResultado`):** os blocos "Falta dado para este cenário" e "Sem dado publicado" saíram do comparador. No lugar, depois do último cartão, a lista "Marcas que não retornaram resultado" com marca + motivo ("Não oferece plano de recebimento na hora", "Sem dado publicado para este cenário: <o que falta>", "não publica tabela…"). Só marcas escolhidas entram; marca com pelo menos um cartão (calculado, promocional ou faixa) não entra. A visão "Tabela de taxas" agora usa só o bloco calculado (o cartão de plano incompleto não existe mais para receber o clique).
- **Nota do Reclame Aqui** alinhada à direita na mesma linha em negrito do "Link de parceiro" (ou da adesão, sem parceria) — `x-nota-reclame-aqui`.
- **FAQ:** primeira pergunta nova "Eu pago ao Máquina Certa? Quem entrega a maquininha e dá suporte?" — pagamento direto à marca, responsabilidade de entrega/suporte é da marca. **Metodologia** ganhou "Como calculamos o custo mensal" (prazo, adesão pelo link, marcas sem resultado, nota do RA mensal), a frase dos pagamentos diretos à marca e o aviso de que Cielo/Rede/GetNet/Stone só entram quando houver relatos suficientes (hoje 0 faixas reportadas em produção).
- **/cupons:** saiu a frase "Todo cupom de adesão vigente hoje…"; cada cartão mostra o logo da marca (`x-bloco-cupom :logo`).
- **Página de marca — tabela de taxas com seletores** (plano, prazo de recebimento, bandeira "Visa e Mastercard"/"Outras bandeiras"), igual ao modal "Ver todas as taxas": as linhas carregam `prazo_codigo`/`bandeira_padrao`/`eh_pix` (`TabelaDeTaxasDaMarca`), `x-tabela-taxas` aplica `x-show` por linha e o Pix aparece sempre. Alpine entra por `resources/js/marca.js` (só o Alpine, sem o motor). A coluna "Prazo" saiu da tabela dessa página.
